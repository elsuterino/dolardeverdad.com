...
(function(window, document, Chartist) {
    ...
    Chartist.plugins.crosshairCursor = function(options) {
        ...
        //private methods
        let sendMetaData, freezeChange, currentPoints, createCrosshairCursor, destroyCrosshairCursor,
            styleCrosshairCursor, unwrapChart, calculatePointArea, showCrosshairCursor, moveCrosshairCursor,
            highlightCurrentPoints, clickFn, hideCrosshairCursor;
        ...
        CrosshairCursor.prototype.move = function(e) {
            if(!this.frozen){
                const position = moveCrosshairCursor.call(this._chartWrapper, e, this._crosshairCursor);
                const highlightedPoints = highlightCurrentPoints(position, this._pointArea);
        ...
        sendMetaData = function(points) {
            const meta = points.map(function (point) {
                return point.meta;
            });
        ...
        createCrosshairCursor = function(chartWrapper, type) {
            ...
            const crosshairCursor = chartWrapper.querySelector('.crosshairCursor-' + type);
        ...
        const inPointArea = function (cursorPosition, pointPosition, padding) {
            return (cursorPosition >= (pointPosition - padding) && cursorPosition <= (pointPosition + padding));
        };

        const formatPointData = function (point) {
            const meta = Chartist.deserialize(point.meta);
        ...
        const toggleHighlight = function (point, highlight) {
            if (highlight) {
        ...
        const cursorIsOnPoint = function (cursorPosition, pointPosition, padding) {
        ...
        calculatePointArea = function() {
            ...
            const pointWidth = window.getComputedStyle(ctPoints[0])['stroke-width'].replace('px', '');
            return +pointWidth/2;
        };
        ...
        highlightCurrentPoints = function(crosshairCursorPosition, pointArea) {
            return dataPoints.filter(function(point) {
                const highlighted = cursorIsOnPoint(crosshairCursorPosition, point.position, pointArea);
        ...
        clickFn = function(e) {
            e.stopPropagation();
            const meta = currentPoints().map(function (point) {
                return point.meta;
            });
